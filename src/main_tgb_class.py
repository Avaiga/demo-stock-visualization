...
class StockVisualization(Page):
    ...
    def __init__(self):
        ...
        self.n_years: int = 1
        self.data: pd.DataFrame = get_stock_data(
            self.selected_stock, self.start_date, self.end_date
        )
    ...
    def get_data_range(self):
        ...
        end_date = (
            ...
            else self.end_date.strftime("%Y-%m-%d")
        )
        ...
        if len(self.data) == 0:
            notify(
                self,
                "error",
                f"Not able to download data {self.selected_stock} from {start_date} to {end_date}",
            )
            return
        ...
        with tgb.layout("1 2 1", gap="40px", class_name="card"):
            ...
            with tgb.part():
                ...
                tgb.input(
                    ...
                    label="Stock",
                    on_action=self.get_data_range,
                )

                tgb.text("or choose a popular one")
                lov = ["MSFT", "GOOG", "AAPL", "AMZN", "META", "COIN", "AMC", "PYPL"]
                tgb.toggle(
                    value="{selected_stock}", lov=lov, on_change=self.get_data_range
                )
            with tgb.part():
                ...
                tgb.button(
                    "Predict",
                    on_action=self.forecast_display,
                    class_name="{'plain' if len(forecast)==0 else ''}",
                )
    ...
    def prediction_part(self):
        with tgb.layout("1 1", class_name="text-center"):
            tgb.text(
                "Pessimistic Forecast {int((forecast.loc[len(forecast)-1, 'Lower'] - forecast.loc[len(data), 'Lower'])/forecast.loc[len(data), 'Lower']*100)}%",
                class_name="h4 card",
            )

            tgb.text(
                "Optimistic Forecast {int((forecast.loc[len(forecast)-1, 'Upper'] - forecast.loc[len(data), 'Upper'])/forecast.loc[len(data), 'Upper']*100)}%",
                class_name="h4 card",
            )
    ...
    def create_page(self):
        with tgb.Page() as page:
            ...
            with tgb.part("container"):
                ...
                with tgb.expandable(
                    title="Historical Data", value="Historical Data", expanded=False
                ):
